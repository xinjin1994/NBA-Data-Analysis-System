...
public class RecentMatchPanel extends JPanel implements DateChangeable {
	...
	private void construct(ArrayList<Date> availables, MatchChangeable matchChanger, MatchItemPanel_Small pnl_item) {
		...
		/*
		 * ShortDate today = new ShortDate(); ShortDate day = new
		 * ShortDate(availables.get(0)); ShortDate[] datelist;
		 * if(today.equals(day)){ datelist = new ShortDate[availables.size()];
		 * for(int i = 0;i < datelist.length;i++) datelist[i] = new
		 * ShortDate(availables.get(i)); } else{ datelist = new
		 * ShortDate[availables.size()+1]; datelist[0] = today; for(int i = 0;i
		 * < availables.size();i++) datelist[i+1] = new
		 * ShortDate(availables.get(i)); }
		 */
		...
		try {
			...
			// ((CardLayout)pnl_view.getLayout()).show(pnl_view,
			// NO_MATCH_TODAY);
